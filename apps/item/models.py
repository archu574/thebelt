...
class ItemManager(models.Manager):
    ...
    def adding_to_wishlist(self, user_id, item_id):
        item = Item.objects.get(id=item_id)
        user = User.objects.get(id=user_id)

        item.wished_by.add(user)

    def removing_from_wishlist(self, user_id, item_id):
        item = Item.objects.get(id=item_id)
        user = User.objects.get(id=user_id)

        item.wished_by.remove(user)
    def showingitem(self,user_id,item_id):
        pass
...
class Item(models.Model):
    ...
    wished_by = models.ManyToManyField(User, related_name="wishfor")
    objects = ItemManager()
